...
def sendMesssageToWatson(request):
    authenticator = IAMAuthenticator('K3n_iM4Idxx75nXSq5w8elE3b-Zh9sWP0foNWdDJvDdT')
    assistant = AssistantV2(
    version='2018-09-20',
    authenticator=authenticator)
    assistant.set_service_url('https://api.eu-gb.assistant.watson.cloud.ibm.com/instances/2b337e35-99f5-408f-96ce-703833386e42')



    session = assistant.create_session("c435fab7-6ff6-45c3-8b93-40522268a383").get_result()
    SessionString = json.dumps(session, indent=2);
    sesstionJson = json.loads(SessionString)
    sessionId = sesstionJson["session_id"]



    message = assistant.message(
      "c435fab7-6ff6-45c3-8b93-40522268a383",
      ""+sessionId,
      input={'text': ''+request},
      context={
          'metadata': {
    ...
    responseString = json.dumps(message, indent=2);
    responseJson = json.loads(responseString)
    response = responseJson["output"]["generic"][0]["text"]

    print(response)

    ## add the TTS here (you need to play mp3 using py)
